...
var opts = require('optimist')
        ...
        .default('c', path.resolve(lib.core.paths.src + '/config/main.js'))
        .argv;
...
client.connect(function(err) {
    ...
    qtt.on('end', function() {
        for (var t in tables) {
            if (modelName !== null && modelName !== t.singular())
                continue;
        ...
        setTimeout(function() {
            ...
            var _generateSkeleton = function() {
                for (tt in tables) {
                    if (modelName !== null && modelName !== tt.singular())
                        continue;
                    ...
                    fs.mkdir(path.resolve(lib.core.paths.src + '/views/' + model.camelName), 0755);
                    ...
                    var modelFile = path.resolve(lib.core.paths.src + '/models/' + model.camelName + '.js');
                    var controllerFile = path.resolve(lib.core.paths.src + '/controllers/' + model.camelName + 'Controller.js');
                    var restControllerFile = path.resolve(lib.core.paths.src + '/controllers/rest/' + model.camelName + 'Controller.js');
                    var angularControllerFile = path.resolve(lib.core.paths.public + '/js/angular/controllers/' + model.camelName + 'Controller.js');
                    var listViewFile = path.resolve(lib.core.paths.src + '/views/' + model.camelName + '/list.ejs');
                    var formViewFile = path.resolve(lib.core.paths.src + '/views/' + model.camelName + '/form.ejs');
                ...
                var app = {};
                app.name = appName;
                app.secret = Math.ceil(Date.now() * Math.random() * 1000000000);
                ...
                var mainFile = path.resolve(lib.core.paths.src + '/config/main.js');
                var routesFile = path.resolve(lib.core.paths.src + '/config/routes.js');
                var angularResFile = path.resolve(lib.core.paths.public + '/js/angular/resourceService.js');
                var layoutMainFile = path.resolve(lib.core.paths.src + '/views/layouts/main.ejs');
                var layoutHeaderFile = path.resolve(lib.core.paths.src + '/views/layouts/header.ejs');
                var layoutFooterFile = path.resolve(lib.core.paths.src + '/views/layouts/footer.ejs');
                var sidebarFile = path.resolve(lib.core.paths.src + '/views/elements/sidebar.ejs');
            ...
            // generate dirs
            fs.mkdir(path.resolve(lib.core.paths.public), 0755, function(err) {
                fs.mkdir(path.resolve(lib.core.paths.public + '/js'), 0755, function() {
                    fs.mkdir(path.resolve(lib.core.paths.public + '/js/angular'), 0755, function() {
                        fs.mkdir(path.resolve(lib.core.paths.public + '/js/angular/controllers'), 0755);
                    });
                });
                fs.mkdir(path.resolve(lib.core.paths.public + '/css'), 0755);
                fs.mkdir(path.resolve(lib.core.paths.public + '/img'), 0755);

                fs.mkdir(path.resolve(lib.core.paths.assets), 0755, function(err) {
                    fs.mkdir(path.resolve(lib.core.paths.assets + '/less'), 0755, function() {
                        fs.mkdir(path.resolve(lib.core.paths.src + '/models'), 0755, function() {
                            fs.mkdir(path.resolve(lib.core.paths.src + '/controllers'), 0755, function(err) {
                                fs.mkdir(path.resolve(lib.core.paths.src + '/controllers/rest'), 0755, function() {
                                    fs.mkdir(path.resolve(lib.core.paths.src + '/views'), 0755, function() {
                                        fs.mkdir(path.resolve(lib.core.paths.src + '/views/layouts'), 0755, function() {
                                            fs.mkdir(path.resolve(lib.core.paths.src + '/views/elements'), 0755, _generateSkeleton);

                                            // copy rest of files
                                            shell.cp('-R', path.resolve(__dirname + '/skeleton/assets/less/*.less'), path.resolve(lib.core.paths.src + '/assets/less'));
                                            shell.cp('-R', path.resolve(__dirname + '/skeleton/assets/css/*.css'), path.resolve(lib.core.paths.public + '/css'));
                                            shell.cp('-R', path.resolve(__dirname + '/skeleton/assets/js/*.js'), path.resolve(lib.core.paths.public + '/js'));
                                            shell.cp('-R', path.resolve(__dirname + '/skeleton/assets/js/angular/*.js'), path.resolve(lib.core.paths.public + '/js/angular'));
                                            shell.cp('-R', path.resolve(__dirname + '/skeleton/assets/img/*.*'), path.resolve(lib.core.paths.public + '/img'));
                                            shell.cp('-R', path.resolve(lib.core.paths.lib + '/controllers/baseController.js'), path.resolve(lib.core.paths.src + '/controllers'));
                                            shell.cp('-R', path.resolve(lib.core.paths.lib + '/controllers/rest/baseController.js'), path.resolve(lib.core.paths.src + '/controllers/rest'));
                                        });
                                    });
                                });
                            });
